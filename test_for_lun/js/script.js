...
var cat = $('.item__cat');
var dog = $('.item__dog'); dog.on('click', function () {
	$('.error_cat').empty();
...
var finalScreen = $('.final'); finalBtn.on('click', function () {
	...
	imgSelect.addClass('final-img');
	if (imgSelect !== undefined) {
		$('.container').css('display', 'none');
		finalWrap.css('display', 'block');
		finalScreen.append(imgSelect).append('<h2 class="last">' + user.name + '</h2>').append('<p class="last">' + user.email + '</p>').append('<p class="last">' + user.country + ',' + ' ' + user.citie);
